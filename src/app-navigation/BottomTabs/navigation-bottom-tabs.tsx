...
const Tabs = [
  {
    ...
    icon: (color: string) => <Entypo name="home" color={color} size={sizes._24sdp} />,
  },
  {
    name: MainTab.Search,
    screen: require('@app-views/Search/Search').default,
    icon: (color: string) => <AntDesign name="search" color={color} size={sizes._24sdp} />,
  },
  {
    name: MainTab.Order,
    screen: require('@app-views/Order/OrderList').default,
    icon: (color: string) => <Ionicons name="receipt-outline" color={color} size={sizes._24sdp} />,
  },
  {
    name: MainTab.Personal,
    screen: require('@app-views/Personal/Personal').default,
    icon: (color: string) => <AntDesign name="user" color={color} size={sizes._24sdp} />,
  },
];
...
const BottomTabs: React.FC<Props> = ({ route }) => {
  ...
  return (
    <Tab.Navigator screenOptions={{ headerShown: false }} initialRouteName={initialScreenName} tabBar={CustomTabar}>
      {Tabs.map(screen => (
